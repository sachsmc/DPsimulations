...
marg.dt <- fread("C:/Users/micsac/Desktop/server-margs/new-marginals.csv")
...
for(j in seeds[-c(1:121)]) {
  A <- marg.dt[trial.leftout < 13 & seed == j][, .SD[sample(1:nrow(.SD), nrow(.SD), replace = FALSE),], by = .(scenario, trial.leftout)]
  ...
  setkey(tomerge, scenario, trial.leftout, inseed)
  
  
  res.dt[, c("t.hat.post", "seed", "yhat", "yhat.lower", "yhat.upper", "i.seed", "predy.ideal", "absdiff.ideal") := NULL]
  
  gc()
  setkey(res.dt, scenario, trial.leftout, inseed)
  res.dt <- merge(res.dt, 
                  tomerge[, .(scenario, trial.leftout, inseed, varset.minD, absdiff.minD = absdiff)], by = c("scenario", "trial.leftout", "inseed"))
...
save(res.fin, file = "res.fin.RData")
...
resdt.sel$mean.excess <- NA
resdt.sel$true.excess <- NA
for(i in 1:nrow(resdt.sel)){
  ...
  resdt.sel$mean.excess[i] <- resdt.sel$corsel.mean.plus[i] * (length(strsplit(resdt.sel$sel.mean[i], "|", fixed = TRUE)[[1]]) - 
                                                                 length(strsplit(resdt.sel$true.vars[i], "|", fixed = TRUE)[[1]]))
  resdt.sel$true.excess[i] <- resdt.sel$corsel.true.plus[i] * (length(strsplit(resdt.sel$sel.trueD[i], "|", fixed = TRUE)[[1]]) - 
                                                                 length(strsplit(resdt.sel$true.vars[i], "|", fixed = TRUE)[[1]]))
}

resdt.sel[, .(mean(nullsel.mean)), by = scenario]

 dpp.simtab2 <- resdt.sel[, .(cor.mean = mean(corsel.mean), 
                            cor.mean.plus = mean(corsel.mean.plus), 
                            mean.excess = mean(mean.excess[mean.excess > 0]),
                            true.excess = mean(true.excess[true.excess > 0]),
...
cover.dpp <- checkcover[, .(coverage = mean(covered), cover.width = mean(abs(yhat.upper - yhat.lower) + 2 * mean.D),  
                               mean.Dhat = mean(absdiff.true), 
                               mean.D = mean(mean.D), sd.D = sd(mean.D)), by = "scenario"]

#save(cover.linear, file = "sim-table1-linear.RData")


tab2.res <- cbind(cover.dpp[, .(scenario, sprintf("%.2f (%.2f)", coverage, cover.width), sprintf("%.2f", mean.Dhat), 
                                sprintf("%.2f (%.2f)", mean.D, sd.D))])

knitr::kable(tab2.res)
...
db <- geom_abline(xintercept = 0, slope = 1, color = "grey")
li <- c(-1, 3)
